    ...
    for ii in range(len(all_graph_event_links)):
        ...
        for jj in range(len(curr_graph_event_entity_relations)):
            ...
            if temp_event not in curr_graph_mentioned_events:
                continue
            # print(curr_graph_event_entity_relations[jj])
...
def _construct_graph_init(events, entities, event_entity_relas, event_links):
    ...
    for ii in range(len(events)):
        ...
        # node_dict_reverse[ii] = event_id
        # temp_feature = np.array(node_type_embeddings[node_type_ontology[event_type]])
    ...
    for ii in range(len(event_entity_relas)):
        ...
        entity_type = entity_type_dict[entity_id]
        # temp_feature = np.array(node_type_embeddings[node_type_ontology[entity_type]])
...
def _construct_graph(pruned_file):
    ...
    for ii in range(len(all_graph_events)):
        ...
        entities = all_graph_entities[ii]
        entity_relas = all_graph_entity_relations[ii]
        print(len(events), len(event_entity_relas), len(event_links), len(entities), len(entity_relas))
        ...
        # print(dis_matrix[ii][jj])

        # if len(np.nonzero(A_true - A_init)[0]) == 0:
        #     print(A_true.shape)
        #     print(np.nonzero(A_true - A_init))
        #     print("equal")
        #     raise
...
def read_dataset():
    ...
    for scenario in scenarios:
        print(scenario)
        train_A_init, train_A_true, train_x_features, train_len, train_dis_matrix, train_role_f = _construct_graph(
            '../../data/Wiki_IED_split/train/' + scenario +
                'train_pruned_new_no_iso_max_50_dataset_enriched.pkl')
        print(len(train_A_init))
        dev_A_init, dev_A_true, dev_x_features, dev_len, dev_dis_matrix, dev_role_f = _construct_graph(
            '../../data/Wiki_IED_split/dev/' + scenario +
                'dev_pruned_new_no_iso_max_50_dataset_enriched.pkl')
        test_A_init, test_A_true, test_x_features, test_len, test_dis_matrix, test_role_f = _construct_graph(
            '../../data/Wiki_IED_split/test/' + scenario +
                'test_pruned_new_no_iso_max_50_dataset_enriched.pkl')
        all_train_A_init += train_A_init
        all_train_A_true += train_A_true
        all_train_x_features += train_x_features
        all_train_len += train_len
        all_train_dis += train_dis_matrix
        all_train_role += train_role_f

        all_dev_A_init += dev_A_init
        all_dev_A_true += dev_A_true
        all_dev_x_features += dev_x_features
        all_dev_len += dev_len
        all_dev_dis += dev_dis_matrix
        all_dev_role += dev_role_f
    ...
    with open('../../data/train_pruned_with_bert_max_50_set_enriched.pkl', 'wb') as handle:
        pickle.dump([all_train_A_init, all_train_A_true, all_train_x_features, all_train_len, all_train_dis, all_train_role], handle)
    with open('../../data/dev_pruned_with_bert_max_50_set_enriched.pkl', 'wb') as handle:
        pickle.dump([all_dev_A_init, all_dev_A_true, all_dev_x_features, all_dev_len, all_dev_dis, all_dev_role], handle)
    with open('../../data/test_pruned_with_bert_max_50_set_enriched.pkl', 'wb') as handle:
...
#     for scenario in scenarios:
#         train_A_init, train_A_true, train_x_features, train_len = _construct_graph(
#             '../../data/Wiki_IED_split/train/' + scenario +
#                 'train_pruned_new_no_iso_max_50_dataset.pkl')
#         dev_A_init, dev_A_true, dev_x_features, dev_len = _construct_graph(
#             '../../data/Wiki_IED_split/dev/' + scenario +
#                 'dev_pruned_new_no_iso_max_50_dataset.pkl')
#         test_A_init, test_A_true, test_x_features, test_len = _construct_graph(
#             '../../data/Wiki_IED_split/test/' + scenario +
...
#         pickle.dump([all_train_A_init, all_train_A_true, all_train_x_features, all_train_len], handle)
#     with open('../../data/dev_pruned_with_bert_max_50_set.pkl', 'wb') as handle:
#         pickle.dump([all_dev_A_init, all_dev_A_true, all_dev_x_features, all_dev_len], handle)
#     with open('../../data/test_pruned_with_bert_max_50_set.pkl', 'wb') as handle:
#         pickle.dump([all_test_A_init, all_test_A_true, all_test_x_features, all_test_len], handle)
...
# scenarios = ["wiki_ied_bombings_", "suicide_ied_", "wiki_drone_strikes_","wiki_mass_car_bombings_"]
# for scenario in scenarios:
#     calcu_role_info('../../data/Wiki_IED_split/train/' + scenario +'train_pruned_new_no_iso_max_50_dataset.pkl')
