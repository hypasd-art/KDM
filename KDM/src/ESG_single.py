...
def test(model, dataloader, args, idx=None):
    model.eval()
    with torch.no_grad():
        if idx == None:
            model.sample_test_single(args.training_sample_to_save)
        else:
            model.sample_test(idx)
    torch.cuda.empty_cache()

def main(args):
    if args.predictor:
        if args.data_type == "IGE":
            model = torch.load(args.save+"/model_{}_expansion.pt".format(args.dataset))
        elif args.data_type == "json":
            model = torch.load(args.save+"/model_{}_expansion_json.pt".format(args.dataset))
        elif args.data_type == "None":
            model = torch.load(args.save+"/model_{}.pt".format(args.dataset))

        model.eval()
        _, test_list = load_graphs_with_type_label_single(args, name = args.dataset)

        with torch.no_grad():
            model.sample_test_single(args.testing_sample_to_save)
            evaluation_generation_res(args.dataset, args, seed=args.seed)

    else:
        train_list, test_list = load_graphs_with_type_label_single(args, name = args.dataset)
        print("*****************************creating dataset******************************")
        train_dataset = GraphDataset_single(args, train_list)
        ...
        test_dataset= GraphDataset_single(args, test_list)
        ...
        for epoch in range(args.epochs):
            ...
            if epoch % 100 == 0:
                result = test(model, test_dataloader, args)
                evaluation_generation_res(args.dataset, args, seed=args.seed)

                if args.data_type == 'None':
                    torch.save(model, args.save+"/model_{}.pt".format(args.dataset))
                    print("model saved")
                elif args.data_type == 'IGE':
                    torch.save(model, args.save+"/model_{}_expansion.pt".format(args.dataset))
                elif args.data_type == 'json':
                    torch.save(model, args.save+"/model_{}_expansion_json.pt".format(args.dataset))
